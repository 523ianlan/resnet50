"""Common helpers for paper table experiments."""
...
def apply_paper_defaults(cfg: PruningConfig) -> None:
    """Apply paper-specified training defaults."""
    cfg.fine_tune_epochs = 10
    cfg.base_lr = 1e-4
    cfg.fine_tune_lr = 1e-4
    cfg.weight_decay = 1e-4
    cfg.fine_tune_weight_decay = 1e-4
    cfg.optimizer = {
        "name": "adamw",
        "betas": [0.9, 0.999],
        "eps": 1e-8,
...
def write_overrides(exp_dir: str, name: str, overrides: Dict[str, Any]) -> None:
    ...
    payload = {"name": name, "overrides": overrides}
